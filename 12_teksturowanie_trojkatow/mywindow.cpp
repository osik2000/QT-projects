...
void MyWindow::on_loadButton_clicked()
{
   nazwa_pliku = QFileDialog::getOpenFileName(this,
    tr("Zaladuj Obraz"), "..\\12_teksturowanie_trojkatow", tr("Typy plikow graficznych (*.png *.jpg *.bmp)"));
   if (nazwa_pliku != ""){
...
void MyWindow::rysuj_punkty(){
    ...
    if(licznikpunktow == 3 && licznikpunktow2 == 3) teksturuj();
...
void MyWindow::teksturuj(){
    ...
    for(int i = 1; i < 3; i++){
        ...
        if(punkty_x2[i] < minx) minx = punkty_x2[i];
        if(punkty_y2[i] < miny) miny = punkty_y2[i];
        if(punkty_x2[i] > maxx) maxx = punkty_x2[i];
        if(punkty_y2[i] > maxy) maxy = punkty_y2[i];
    }

    mianownik = (((punkty_x2[1] - punkty_x2[0]) * (punkty_y2[2] - punkty_y2[0])) - ((punkty_y2[1] - punkty_y2[0]) * (punkty_x2[2] - punkty_x2[0])));

    if (interpolacja){

        for(int iy = miny; iy < maxy; iy++){
            for(int ix = minx; ix < maxx; ix++){
                v = ((( (ix - punkty_x2[0]) * (punkty_y2[2] - punkty_y2[0]) ) - ((punkty_x2[2] - punkty_x2[0]) * ((iy - punkty_y2[0])))) / mianownik);
                w = (( ( (punkty_x2[1] - punkty_x2[0]) * (iy - punkty_y2[0]) ) - ((punkty_y2[1] - punkty_y2[0]) * (ix - punkty_x2[0]))) / mianownik);
                u = 1 - v - w;
                ...
                if(u > 0 && u < 1 && v > 0 && v < 1 && w > 0 && w < 1){
                    ...
                    rysuj_pikselPunkty(ix,iy,colort,"right");
                }
            }
        }
        ...
        for(int iy = miny; iy < maxy; iy++){
            for(int ix = minx; ix < maxx; ix++){
                v = ((((ix - punkty_x2[0]) * (punkty_y2[2] - punkty_y2[0])) - ((iy - punkty_y2[0]) * (punkty_x2[2] - punkty_x2[0]))) / mianownik);
                w = ((((punkty_x2[1] - punkty_x2[0]) * (iy - punkty_y2[0])) - ((punkty_y2[1] - punkty_y2[0]) * (ix - punkty_x2[0]))) / mianownik);
                u = 1 - v - w;
                ...
                if(u > 0 && u < 1 && v > 0 && v < 1 && w > 0 && w < 1){
                    ...
                    xt = u * punkty_x[0] + v * punkty_x[1] + w * punkty_x[2];
                    yt = u * punkty_y[0] + v * punkty_y[1] + w * punkty_y[2];
                    colort = pobierz_kolor(img, (int)xt, (int)yt, "left");
                    rysuj_pikselPunkty(ix,iy,colort,"right");
                }
            }
        }
...
void MyWindow::on_interpolacjaCheckBox_stateChanged(int arg1)
{
    if (arg1) interpolacja = true;
    else interpolacja = false;
